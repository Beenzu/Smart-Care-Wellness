...
async function submitOrder(formEl) {
  ...
  const qty = parseInt(data.get('qty'), 10);
  ...
  if (!Number.isInteger(qty) || qty < 1 || qty > product.qty) {
    showToast('Please enter a valid quantity available in stock');
    return;
  }
...
function startOrder(id) {
  ...
  if (product.qty <= 0) { showToast('This product is currently out of stock'); return; }
...
function setOrderQty(value) {
  ...
  if (!product || !Number.isInteger(qty)) return;
  qty = Math.max(1, Math.min(product.qty, qty));
...
function renderCheckout(app) {
  ...
  if (!p || p.qty <= 0) { cancelOrder(); return; }
  const qty = Math.max(1, Math.min(p.qty, state.orderDraft.qty || 1));
  state.orderDraft.qty = qty;
      ...
      <div class="checkout-page">
        ...
        <div class="checkout-layout">
          <div class="panel order-summary-card">
            ...
            <div class="field"><label for="order-qty">Quantity</label><div class="qty-selector"><button type="button" onclick="setOrderQty(${qty - 1})" ${qty <= 1 ? 'disabled' : ''}>−</button><input id="order-qty" type="number" min="1" max="${p.qty}" value="${qty}" onchange="setOrderQty(this.value)"><button type="button" onclick="setOrderQty(${qty + 1})" ${qty >= p.qty ? 'disabled' : ''}>+</button></div><small class="muted">${p.qty} available</small></div>
