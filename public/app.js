...
const state = {
  view: 'store', // store | confirm | admin-auth | admin | about | services
  loading: true,
  products: [],
  categories: [],
  activeCategory: 'All',
  ...
  hasAdmin: false,
  isAdmin: false,
  adminTab: 'stock', // stock | orders | services
  settings: { storeName: 'Godwyn Stores', tagline: 'Quality goods, delivered across Zambia.', currency: 'ZMW', whatsappNumber: '0760565047', supportPhone: '0760565047', aboutText: '' },
  cartOpen: false,
  toast: null,
...
async function init() {
  try {
    const [products, settings, adminStatus, categories, services] = await Promise.all([
      api('/api/products'),
      api('/api/settings'),
...
function topbar() {
  ...
  <div class="topbar">
    ...
    <nav class="main-nav">
      <a href="#" class="${state.view==='store'?'active':''}" onclick="event.preventDefault(); state.view='store'; render();">Shop</a>
      <a href="#" class="${state.view==='about'?'active':''}" onclick="event.preventDefault(); state.view='about'; render();">About</a>
      <a href="#" class="${state.view==='services'?'active':''}" onclick="event.preventDefault(); state.view='services'; render();">Services</a>
    </nav>
  ...
  app.innerHTML = `
    ...
    <div class="wrap">
      <div class="search-bar">
        ...
        <button class="btn search-btn" onclick="triggerSearch()" aria-label="Search">🔍 Search</button>
      </div>
...
function productCard(p) {
  ...
  <div class="card">
    ...
    <button class="link-btn" onclick="openReviews('${p.id}')">${p.reviewCount ? '★ ' + p.avgRating + ' (' + p.reviewCount + ' review' + (p.reviewCount===1?'':'s') + ')' : 'No reviews yet — be the first'}</button>
    <div class="price-row">
...
function renderAbout(app) {
  ...
  <div class="wrap">
    <div class="panel">
      ...
      <p class="muted" style="white-space:pre-wrap;">${escapeHtml(state.settings.aboutText) || 'We are dedicated to bringing you quality goods, delivered reliably.'}</p>
    </div>
  </div>
  ${toastHtml()}`;
}
