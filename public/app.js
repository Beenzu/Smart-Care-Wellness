...
const state = {
  ...
  hasAdmin: false,
  isAdmin: false,
  adminTab: 'stock', // stock | services
  navOpen: false,
  settings: { storeName: 'Godwyn Stores', tagline: 'Quality goods, delivered across Zambia.', currency: 'ZMW', whatsappNumber: '0760565047', supportPhone: '0760565047', aboutText: '', storeAddress: '', contactEmail: '', facebookUrl: '', instagramUrl: '', tiktokUrl: '' },
  cartOpen: false,
...
async function init() {
  ...
  // Support shareable/ad links such as /order/PRODUCT_ID -> /?order=PRODUCT_ID.
  // The order page should open at the top, not at the previous scroll position.
  const directOrderId = new URLSearchParams(window.location.search).get('order');
  if (directOrderId) {
    const directProduct = state.products.find(p => p.id === directOrderId);
    if (directProduct && Number.isInteger(Number(directProduct.qty)) && Number(directProduct.qty) > 0) {
      state.orderDraft = { productId: directProduct.id, qty: 1 };
      state.view = 'checkout';
      state.navOpen = false;
...
async function submitOrder(formEl) {
  ...
  if (!Number.isInteger(availableStock) || availableStock < 0) {
    showToast('This product has an invalid stock value. Please contact Godwyn Stores.');
    return;
  }
...
function startOrder(id) {
  ...
  state.cartOpen = false;
  render();
  // Mobile browsers may preserve the store's previous scroll position.
  // Always put a newly opened order form at the top.
  window.scrollTo({ top: 0, left: 0, behavior: 'auto' });
}
...
function setOrderQty(value) {
  ...
  if (!product || !Number.isInteger(availableStock) || availableStock < 1) return;

  // Accept both button clicks and typed values, then clamp to valid stock.
  let qty = Number(String(value).trim());
  if (!Number.isFinite(qty)) qty = state.orderDraft.qty || 1;
  qty = Math.round(qty);
  ...
  if (total) total.textContent = money(product.price * qty);

  // The checkout HTML is not re-rendered for every tap (that would reset form
  // fields). Update the +/- controls in place so each tap uses the new value.
  const buttons = document.querySelectorAll('.qty-selector button');
  if (buttons.length >= 2) {
    const minus = buttons[0];
    const plus = buttons[buttons.length - 1];
    minus.disabled = qty <= 1;
    plus.disabled = qty >= availableStock;
    minus.onclick = () => setOrderQty(state.orderDraft.qty - 1);
    plus.onclick = () => setOrderQty(state.orderDraft.qty + 1);
  }
}
...
function copyAdLink(id) {
  const url = window.location.origin + '/order/' + encodeURIComponent(id);
...
function navDrawer() {
  ...
  <div class="nav-drawer ${state.navOpen?'open':''}">
    ...
    <nav class="nav-drawer-links">
      ...
      ${state.isAdmin
        ? `<a href="#" onclick="event.preventDefault(); state.navOpen=false; state.view='admin'; render();">🔐 Dashboard</a>`
        : `<a href="#" onclick="event.preventDefault(); state.navOpen=false; state.view='admin-auth'; render();">🔐 Store Login</a>`}
    </nav>
  </div>`;
}
    ...
    <div class="wrap">
      <section class="store-hero">
        <div class="store-hero-copy">
          <span class="eyebrow">WELCOME TO ${escapeHtml(state.settings.storeName)}</span>
          <h1>Quality products.<br><span>Simple shopping.</span></h1>
          <p>${escapeHtml(state.settings.tagline || 'Quality goods, delivered across Zambia.')}</p>
          <div class="hero-actions">
            <button class="btn hero-primary" onclick="document.getElementById('product-section')?.scrollIntoView({behavior:'smooth'});">Shop products</button>
            <button class="btn ghost hero-secondary" onclick="state.view='services'; render();">How we help</button>
          </div>
        </div>
        <div class="store-hero-art" aria-hidden="true">
      ...
      <section class="trust-strip" aria-label="Store benefits">
        <div><span class="trust-icon">✓</span><div><strong>Quality products</strong><small>Selected for value</small></div></div>
        <div><span class="trust-icon">↗</span><div><strong>Convenient delivery</strong><small>To your preferred location</small></div></div>
        <div><span class="trust-icon">₵</span><div><strong>Pay on delivery</strong><small>For eligible orders</small></div></div>
        <div><span class="trust-icon">✦</span><div><strong>Customer support</strong><small>We're here to help</small></div></div>
      </section>
      ...
      ` : ''}
      <div id="product-section" class="section-head"><h2>${state.activeCategory === 'All' ? 'Available stock' : escapeHtml(state.activeCategory)}</h2><span class="muted">${products.length} item${products.length===1?'':'s'}</span></div>
      ${products.length === 0 ? `<div class="empty-state">No products match your search.</div>` : `<div class="grid">${products.map(productCard).join('')}</div>`}
    </div>
    <footer>
      <div class="footer-contact"><strong>For inquiries call or send a WhatsApp message to ${escapeHtml(state.settings.supportPhone || '0760565047')}</strong></div>
...
function renderCheckout(app) {
  ...
  app.innerHTML = `
        ...
        <div class="checkout-layout">
          <div class="panel order-summary-card">
            <div class="checkout-product">
              ...
              <div><span class="cat-badge">${escapeHtml(p.category || '')}</span><h3>${escapeHtml(p.name)}</h3><div class="price">${money(p.price)}</div></div>
            </div>
            <div class="field"><label for="order-qty">Quantity</label><div class="qty-selector"><button type="button" onclick="setOrderQty(${qty - 1})" ${qty <= 1 ? 'disabled' : ''}>−</button><input id="order-qty" type="number" min="1" max="${availableStock}" value="${qty}" onchange="setOrderQty(this.value)"><button type="button" onclick="setOrderQty(${qty + 1})" ${qty >= p.qty ? 'disabled' : ''}>+</button></div><small class="muted">${availableStock} available</small></div>
          ...
          <div class="panel">
            ...
            <form id="checkout-form" onsubmit="event.preventDefault(); submitOrder(this);">
              ...
              <label class="agreement-check"><input type="checkbox" name="agreement" required> <span>I have read and agree to the <a href="#" onclick="event.preventDefault(); state.view='policy'; render();">Godwyn Stores Policy & Agreement</a>.</span></label>
    ...
    <footer><div class="footer-contact"><strong>For inquiries call or send a WhatsApp message to ${escapeHtml(state.settings.supportPhone || '0760565047')}</strong></div><div class="footer-social">${socialLinksHtml()}</div><div class="footer-note">Payment is on delivery.</div></footer>
    ${toastHtml()}`;
}
  ...
  app.innerHTML = `
    ...
    <div class="wrap">
      ...
      <div class="panel policy-page">
        <h1>Godwyn Stores Policy & Agreement</h1>
        ...
        <h3>1. Ordering</h3>
        <p>When you click <strong>Order Now</strong>, you select the product quantity and provide your contact and delivery information. Submitting an order is a request to purchase the selected product; Godwyn Stores will contact you to confirm the order and delivery details.</p>
        ...
        <p>Orders are confirmed by phone. If we cannot reach you or the details provided are incomplete, delivery may be delayed or the order may not be processed. Customers should contact Godwyn Stores as soon as possible if they need to cancel or change an order before delivery.</p>

        <h3>6. Returns, exchanges and damaged items</h3>
        <p>Customers should inspect products at delivery and report any visible damage, missing item or incorrect product promptly. Returns or exchanges depend on the condition of the product and the circumstances of the issue. Contact Godwyn Stores before returning an item so that the appropriate resolution can be agreed.</p>
        ...
        <h3>9. Agreement</h3>
        <p>By checking the agreement box during checkout and placing an order, you confirm that you have read and accepted this Policy & Agreement. Godwyn Stores may update these terms when necessary; the version displayed on the store at the time of your order applies to that order.</p>

        <div class="policy-contact"><strong>Questions?</strong><br>Call or send a WhatsApp message to ${escapeHtml(state.settings.supportPhone || '0760565047')}.</div>
      </div>
    </div>
    <footer><div class="footer-contact"><strong>For inquiries call or send a WhatsApp message to ${escapeHtml(state.settings.supportPhone || '0760565047')}</strong></div><div class="footer-social">${socialLinksHtml()}</div></footer>
    ${toastHtml()}`;
}
...
function renderConfirm(app) {
  ...
  <div class="wrap">
    <div class="panel" style="max-width:480px;margin:40px auto;text-align:center;">
      ...
      ${state.lastOrderWhatsappUrl ? `<a class="btn" href="${escapeHtml(state.lastOrderWhatsappUrl)}" target="_blank" rel="noopener">For inquiries, send a message on WhatsApp: 0760565047</a>` : `<p class="muted">For inquiries, send a message on WhatsApp: 0760565047</p>`}
      <button class="btn ghost" onclick="state.view='store'; render();">Back to store</button>
    </div>
  </div>`;
}
    ...
    <div class="panel">
      ...
      <div class="about-copy">
        ${state.settings.aboutText
          ...
          : `<p>Godwyn Stores is a customer-focused online shop bringing together practical everyday products in <strong>Electronics, Health Products, Cosmetics and Clothes</strong>. We make shopping simpler by giving customers a convenient way to browse available products, place an order and arrange reliable delivery across Zambia.</p>
...
function renderServicesPage(app) {
  ...
  <div class="wrap">
    ...
    ${state.services.length === 0 ? `<div class="empty-state">No services listed yet.</div>` : state.services.map(s => `
      <div class="panel">
...
function renderAdmin(app) {
  ...
  <div class="wrap">
    ...
    <div class="tabbar">
      ...
      <button class="${state.adminTab==='services'?'active':''}" onclick="state.adminTab='services'; render();">Services</button>
    </div>
