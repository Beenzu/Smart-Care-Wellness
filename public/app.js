...
async function refreshRecommendations() {
  ...
  if (cartIds.length) params.set('cartIds', cartIds.join(','));
...
function addToCart(id) {
  ...
  trackAffinity('cartAdd', p.category);
  render();
  refreshRecommendations();
}
...
function removeFromCart(id) { delete state.cart[id]; render(); refreshRecommendations(); }
function toggleCart(open) { state.cartOpen = open; if (open) state.reviewsOpenFor = null; render(); }
...
async function submitOrder(formEl) {
  ...
  try {
    const result = await api('/api/orders', {
      method: 'POST',
      body: { items, customerName, phone, altPhone, email, town, province, address, deliveryDate, deliveryWindow, notes },
    });
    state.cart = {};
...
function topbar() {
  ...
  <div class="topbar">
    ...
    <div class="topbar-right">
      ${state.view === 'store' ? `<button class="cart-pill" onclick="toggleCart(true)">🧺 <span>${cartCount()}</span></button>` : ''}
      ${state.isAdmin
  ...
  <div class="card product-card">
    ...
    <div class="card-body product-actions">
      ...
      ${out ? `<button class="btn full" disabled>Out of stock</button>` :
        inCart > 0 ? `<div class="qty-row"><div class="stepper"><button onclick="decFromCart('${escapeHtml(p.id)}')">−</button><span>${inCart}</span><button onclick="addToCart('${escapeHtml(p.id)}')">+</button></div><span class="muted">in cart</span></div>` :
        `<button class="btn full" onclick="addToCart('${escapeHtml(p.id)}')">Add to Cart</button>`}
    </div>
  </div>`;
}
      ...
      <div class="product-detail panel">
        ...
        <div class="product-detail-info">
          ...
          <div class="product-detail-actions">
            ${out ? `<button class="btn full" disabled>Out of stock</button>` : inCart > 0 ? `<div class="qty-row"><div class="stepper"><button onclick="decFromCart('${escapeHtml(p.id)}')">−</button><span>${inCart}</span><button onclick="addToCart('${escapeHtml(p.id)}')">+</button></div><button class="btn" onclick="toggleCart(true)">Go to checkout</button></div>` : `<button class="btn full" onclick="addToCart('${escapeHtml(p.id)}')">Add to Cart</button>`}
          </div>
        </div>
      </div>
    ...
    <div class="drawer-body">
      ...
      <form id="checkout-form" onsubmit="event.preventDefault(); submitOrder(this);" style="margin-top:16px;">
        ...
        <div class="form-grid">
          ...
          <div class="field"><label>Province</label>
            <select name="province" required>
              ...
              ${state.provinces.map(p => `<option value="${escapeHtml(p)}">${escapeHtml(p)}</option>`).join('')}
            </select>
          </div>
        </div>
        ...
        <div class="form-grid">
          ...
          <div class="field"><label>Preferred time (optional)</label>
            <select name="deliveryWindow">
              <option value="">No preference</option>
              ${state.deliveryWindows.map(w => `<option value="${escapeHtml(w)}">${escapeHtml(w)}</option>`).join('')}
            </select>
          </div>
        </div>
