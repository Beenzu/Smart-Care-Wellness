...
function setOrderQty(value) {
  ...
  if (input) {
    input.value = qty;
    // The +/- buttons have the target qty baked into their onclick attribute
    // at render time, so after changing qty we must rewrite those attributes
    // (and disabled state) too, or the buttons go stale after one click.
    const minusBtn = input.previousElementSibling;
    const plusBtn = input.nextElementSibling;
    if (minusBtn) {
      minusBtn.setAttribute('onclick', `setOrderQty(${qty - 1})`);
      minusBtn.disabled = qty <= 1;
    }
    if (plusBtn) {
      plusBtn.setAttribute('onclick', `setOrderQty(${qty + 1})`);
      plusBtn.disabled = qty >= availableStock;
    }
  }
