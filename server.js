...
app.get('/api/settings', (_req, res) => {
  const { storeName, tagline, currency, whatsappNumber, supportPhone, aboutText, storeAddress, contactEmail } = store.getData().settings;
  res.json({ storeName, tagline, currency, whatsappNumber, supportPhone, aboutText, storeAddress, contactEmail });
});
app.put('/api/admin/settings', requireAdmin, async (req, res) => {
  const data = store.getData();
  const { storeName, tagline, currency, whatsappNumber, supportPhone, aboutText, storeAddress, contactEmail } = req.body || {};
...
app.post('/api/orders', orderLimiter, async (req, res) => {
  const data = store.getData();
  const { items, customerName, phone, altPhone, town, province, address, deliveryDate, deliveryWindow, notes } = req.body || {};
  ...
  for (const [id, qty] of requested) {
    ...
    resolvedItems.push({ id: product.id, name: product.name, category: product.category, price: product.price, qty });
  }
  ...
  const order = {
    ...
    customerName: name, phone: customerPhone, altPhone: customerAltPhone,
