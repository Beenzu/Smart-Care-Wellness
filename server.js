...
app.post('/api/orders', orderLimiter, async (req, res) => {
  ...
  // Sync the order to QuiverCRM. The order is saved locally first so a
  // temporary CRM outage can never lose the customer's order. Failed syncs
  // are marked pending and retried automatically in the background.
  const crmResult = await syncOrderToCRM(order, data);
  if (crmResult.ok) {
    order.crmSync = { status: 'synced', syncedAt: new Date().toISOString() };
  } else {
    order.crmSync = { status: 'pending', lastError: crmResult.error, lastAttemptAt: new Date().toISOString(), attempts: Number(order.crmSync?.attempts || 0) + 1 };
  }
  await store.save();
  res.json({ orderId: order.id, whatsappUrl: waNumber ? `https://wa.me/${waNumber}?text=${waMessage}` : null, crmSync: order.crmSync.status });
...
/* ---------- Direct order/ad link with social preview metadata ---------- */
app.get('/order/:id', (req, res) => {
  ...
  const pageUrl = `${origin}/order/${encodeURIComponent(product.id)}`;
  ...
  const description = product.description || data.settings.tagline || `Order ${product.name} from ${data.settings.storeName}.`;
...
<meta property="og:image:alt" content="${escapeHtmlServer(product.name)}">` : ''}
<meta property="product:price:amount" content="${product.price}">
<meta property="product:price:currency" content="${escapeHtmlServer(data.settings.currency || 'ZMW')}">
<meta name="twitter:card" content="${absoluteImage ? 'summary_large_image' : 'summary'}">
${absoluteImage ? `<meta name="twitter:image" content="${escapeHtmlServer(absoluteImage)}">` : ''}
</head>
<body>
<p>Opening the order page for ${escapeHtmlServer(product.name)}…</p>
<script>window.location.replace('/?order=' + encodeURIComponent(${JSON.stringify(product.id)}));</script>
<noscript><p><a href="/?order=${encodeURIComponent(product.id)}">Continue to order ${escapeHtmlServer(product.name)}</a></p></noscript>
</body>
</html>`);
});
